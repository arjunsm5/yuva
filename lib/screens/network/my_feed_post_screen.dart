...
class _PostItemState extends State<PostItem> {
  ...
  void initState() {
    ...
    score =
        ((widget.postData['upvotes'] ?? 0) -
            (widget.postData['downvotes'] ?? 0));
    hasUpvoted = false;
    hasDownvoted = false;
    isBookmarked = false;
    print(
      ...
      if (mounted) {
        setState(() {
          hasUpvoted =
              ...
              false;
          hasDownvoted =
              (data?['downvotedBy'] as List<dynamic>?)?.contains(
                currentUserId,
              ) ??
              false;
          isBookmarked =
              (data?['bookmarkedBy'] as List<dynamic>?)?.contains(
                currentUserId,
              ) ??
              false;
        });
  ...
  void _navigateToProfile(BuildContext context) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder:
            (context) => ProfileDetailScreen(userId: widget.postData['userId']),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    String timeAgo =
        widget.postData['createdAt'] != null
            ? timeago.format(
      ...
      children: List.generate(options.length, (index) {
        ...
        return Padding(
          ...
          child: GestureDetector(
            onTap:
                isSelected || userVote != null || isLoading
                    ? null
                    : () => castPollVote(option),
            child: Container(
            ...
            return GestureDetector(
              onTap: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder:
                        (context) => _FullScreenImageViewer(imageUrl: imageUrl),
                  ),
                );
              },
              child: Padding(
      ...
      children: [
        Row(
          children: [
            ...
            // Downvote button (fixed from updateVotewatermark to updateVote)
            IconButton(
